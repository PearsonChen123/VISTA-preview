#!/usr/bin/env python3
"""
Run MonSter on RGBD-500 rotated stereo pairs
...
# Add MonSter to path
MONSTER_PATH = Path("/mnt/h/MonSter")
sys.path.insert(0, str(MONSTER_PATH))
sys.path.insert(0, str(MONSTER_PATH / "core"))
...
def main():
    parser = argparse.ArgumentParser(
        description='Run MonSter on rotated stereo pairs and unrotate results'
    )
    parser.add_argument('--restore_ckpt', type=str, required=True,
                        help='Path to MonSter checkpoint (e.g., mix_all.pth)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/h/RGBD-500/stereo/depth_monster',
    ...
    parser.add_argument('--shift_direction', type=str, default='y',
                        choices=['x', '-x', 'y', '-y'],
    ...
    if args.save_disparity:
        disp_dir = output_dir.parent / "disp_monster"
        disp_dir.mkdir(parents=True, exist_ok=True)

    if args.save_visualization:
        vis_dir = output_dir.parent / "depth_monster_vis"
        vis_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print("MonSter Inference for RGBD-500")
    print("=" * 70)
    ...
    with torch.no_grad():
        for left_path, right_path in tqdm(list(zip(left_images, right_images)),
            ...
            depth = disparity_to_depth(disp, K_matrix, baseline)
            ...
            if args.save_disparity:
                disp_unrotated = unrotate_image(disp, rotation_type)
